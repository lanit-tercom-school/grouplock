#pragma once
/*
Библиотека для разделения секрета, где секрет выглядит как квадратная матрица.
Основные методы:
getPartsK - возвращает объект класса  parts, в котором есть два вектора: X, который содержит точки, в которых мы ищем
значение полинома и K_i - сами значения полинома в точках, содержащихся в X. Так же в этом классе содержатся степень
полинома, простое число, количество частей, необходимое для восстановления секрета. Все матрицы Х - диагональные!
На вход принимает минимальное количество ключей - 1, секрет, размерность секрета, количество ключей на которое мы хотим
поделить наш секрет.
makeSecret - "собирает" исходный секрет. На вход принимает ключи, точки (в которых высчитывались эти ключи), простое число
(которое задает конечное поле), размерность секрета (или либого из ключа)
Метод возвращает квадратную матрицу - исходный секрет.
			***
Библиотека основана на типе данных int, поэтому максимальное значение каждого элемента секрета должно быть меньше 1000.
(Возможно в дальнейшем будет переписана на шаблоны и добавится возможность использования более больших чисел. Плюс для
экономии памяти возможна модификация до верхнетреугольных матриц.)
Иванова Марина. Апрель 2016.
*/
#include "Debug\Poly.h"
#include "Debug\PrimeNum.h"
